...
def empezar():
   ...
   def verificar_bingo():
     if not any(verificar(lista_bingos[i], lista_puntos, "X") for i in range(len(lista_bingos))):
         ...
         G = Label(ganaste, text="¡¡BINGO!! \n Tus bingos ganadores son:",fg="white",bg='gray29',font=("w",30)).pack()
         ganadores = []

         for matriz in lista_bingos:
...
Limpiar = Button(ventana, text="Limpiar",command=limpiar).place(x=140,y=400)
#imprimir = Button(ventana, text="Imprimir",command=imprimir).place(x=80, y=400)
Ingresar_forma = Button(ventana, text="Ingresa forma",command=ingresar_forma ).place(x=10, y=50)
ventana.mainloop()
